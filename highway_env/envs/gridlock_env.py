...
class GridlockEnv(AbstractEnv):
    ...
    def _make_road(self) -> None:
        ...
        left_turn_radius = right_turn_radius - 5 # [m}
        outer_distance = right_turn_radius + lane_width / 2 + self.DISTANCE_BETWEEN_ROADS
        access_length = self.ROAD_LENGTH  # [m]
        lateral_offset = lane_width / 2 + self.DISTANCE_BETWEEN_ROADS
        ...
        for corner in range(self.NUM_ROADS):
            ...
            net.add_lane("o" + str(corner), "ir" + str(corner),
                         StraightLane(start, end, line_types=[c, c], priority=priority, speed_limit=10))
            # Right turn
            ...
            # Left turn
            l_center = rotation @ (np.array([-left_turn_radius + lane_width / 2 + self.DISTANCE_BETWEEN_ROADS, left_turn_radius - lane_width / 2 - self.DISTANCE_BETWEEN_ROADS]))
            net.add_lane("ir" + str(corner), "il" + str((corner + 1) % self.NUM_ROADS),
                         CircularLane(l_center, left_turn_radius, angle + np.radians(0), angle + np.radians(-90),
                                      clockwise=False, line_types=[s, n], priority=priority - 1, speed_limit=10))
            # Straight
            ...
            net.add_lane("il" + str((corner - 1) % self.NUM_ROADS), "o" + str((corner - 1) % self.NUM_ROADS),
                         StraightLane(end, start, line_types=[c, c], priority=priority, speed_limit=10))
